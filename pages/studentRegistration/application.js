...
function Application({studentRegistration,renderBody}){
        ...
        const headers = ['First Name', 'Last Name','DOB','Nationality','Email','Phone Number']
        const val = []
        const ids = []
        allValues.map((value) =>{
            const {firstName, lastName, email, phoneNumber,dob, nationality} = value
            val.push(Object.values({firstName, lastName, email, phoneNumber,dob,nationality}));
            ids.push(value.id)
        })
        const color = "light";  
        return (
